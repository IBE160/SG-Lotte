...
**Technical Notes:** This involves setting up the `backend/` directory, `requirements.txt`, basic FastAPI app structure, and Supabase client. The backend should use `uv` for package and project management.
...
**Technical Notes:** Implementation of the background processing logic in FastAPI triggered by Vercel Cron Job (ADR-001), integration with Pydantic AI framework with Gemini 2.5, and database write operations.
