...
Story: story-3.4 - Plan Interruption Management
...
1.  **Missing Architecture Document Citation:** The story does not cite the main architecture document (`docs/architecture-2025-11-30.md`), which is highly relevant for understanding the frontend UI, backend API, and AI integration for plan interruption management.
...
2.  **Incorrect Task-AC Mapping:** Tasks are not explicitly linked to the Acceptance Criteria they fulfill using the required `(AC: #{{ac_num}})` format.
    *   **Evidence:** The task `Implement UI to specify start and end dates for plan pause.` should be formulated like `Task: Implement UI to specify start and end dates for plan pause. (AC: #3.4.1)`.

3.  **Insufficient Testing Subtasks:** The story has 5 acceptance criteria but only two generic testing tasks. Each AC should have corresponding, more granular testing tasks.
    *   **Evidence:** Only two "Testing" tasks exist for five distinct acceptance criteria.
