...
Story: story-1.2 - Core Frontend Setup
...
1.  **Missing Architecture Document Citation:** The story does not cite the main architecture document (`docs/architecture-2025-11-30.md`), which is crucial for understanding the frontend setup and its place in the overall system.
...
2.  **Incorrect Task-AC Mapping:** Tasks are not explicitly linked to the Acceptance Criteria they fulfill using the required `(AC: #{{ac_num}})` format.
    *   **Evidence:** The task `Create Next.js app with TypeScript and Tailwind CSS.` should be formulated like `Task: Create Next.js app with TypeScript and Tailwind CSS. (AC: #1.2.1)`.
