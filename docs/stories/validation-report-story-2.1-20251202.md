...
Story: story-2.1 - Workout Logging UI
...
1.  **Missing Architecture Document Citation:** The story does not cite the main architecture document (`docs/architecture-2025-11-30.md`), which is relevant for understanding the frontend and backend components of the workout logging UI.
...
2.  **Incorrect Task-AC Mapping:** Tasks are not explicitly linked to the Acceptance Criteria they fulfill using the required `(AC: #{{ac_num}})` format.
    *   **Evidence:** The task `Implement UI to mark workouts as "Completed" or "Skipped".` should be formulated like `Task: Implement UI to mark workouts as "Completed" or "Skipped". (AC: #2.1.1)`.
