...
Story: story-3.3 - Account Management
...
1.  **Missing Architecture Document Citation:** The story does not cite the main architecture document (`docs/architecture-2025-11-30.md`), which is highly relevant for understanding the secure account management features utilizing Supabase Auth.
...
2.  **Incorrect Task-AC Mapping:** Tasks are not explicitly linked to the Acceptance Criteria they fulfill using the required `(AC: #{{ac_num}})` format.
    *   **Evidence:** The task `Implement UI for changing password.` should be formulated like `Task: Implement UI for changing password. (AC: #3.3.1)`.

3.  **Insufficient Testing Subtasks:** The story has 4 acceptance criteria but only one generic E2E testing task. Each AC should have corresponding, more granular testing tasks.
