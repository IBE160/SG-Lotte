...
Story: story-2.2 - Meal Logging UI
...
1.  **Missing Architecture Document Citation:** The story does not cite the main architecture document (`docs/architecture-2025-11-30.md`), which is relevant for understanding the frontend and backend components of the meal logging UI.
...
2.  **Incorrect Task-AC Mapping:** Tasks are not explicitly linked to the Acceptance Criteria they fulfill using the required `(AC: #{{ac_num}})` format.
    *   **Evidence:** The task `Implement UI to mark meals as "Eaten" or "Skipped".` should be formulated like `Task: Implement UI to mark meals as "Eaten" or "Skipped". (AC: #2.2.1)`.
