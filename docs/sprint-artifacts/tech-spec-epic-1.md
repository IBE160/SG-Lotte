...
*   **Backend:**
    ...
    *   `app/services/ai_plan_generator.py`: Orchestrates the initial AI plan generation using the Pydantic AI framework with Gemini 2.5, based on user preferences.
*   **Frontend:**
...
3.  **Initial AI Plan Generation & Display (Story 1.5):**
    ...
    *   Backend (`ai_plan_generator.py`) uses user preferences to prompt Gemini 2.5.
...
*   **Backend to AI Service:** Integration with Pydantic AI framework using Gemini 2.5 for plan generation.
...
1.  **Given** I have completed the onboarding flow **When** my dashboard loads **Then** a 7-day personalized workout and meal plan is generated by the AI (Pydantic AI framework with Gemini 2.5).
...
| 1.5.1 | `epics.md` (S1.5), Backend (ai_plan_generator.py) | Backend `/api/v1/plans/generate/`, Gemini 2.5, Pydantic AI   | Integration Test: AI generates plan after onboarding.     |
...
*   **Assumption:** The AI framework (Pydantic AI with Gemini 2.5) will reliably generate valid and structured plan data suitable for direct integration.
